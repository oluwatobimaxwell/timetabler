...
def evaluateSchedule(df_ven, df_veri, visiting, lecturers_id, students_id, courseunit, lecturerFree, studentNotFree, venueData, metadata, trackrecord):
                ...
                if lecPeriod <= timeDiff(stdDay[2], stdDay[1]) and lecPeriod <= timeDiff(lectDay[2], lectDay[1]):
                    if lectDay[1] >= stdDay[1]:
                        ...
                        if len(trackrecord) > 0:
                            for p in trackrecord:
                                if A == p[1] and B == p[2] and C == p[0]:
                                    tracker += 1
    ...
    if not(schedule['status']):
        if metadata < 10 and len(lecturers_id)>0:
            if metadata < 7:
        ...
        schedule['venue'] = scheduleMeetingVenue(df_ven, venueData, schedule['start'], schedule['end'], schedule['day'], 0, [4,5,2,10,3,1], trackrecord)
        # print(schedule['venue'])
        if schedule['venue'] == 'No Free Venue':
            # schedule['start'] = trackrecord[0][1]
            # schedule['end'] = trackrecord[0][2]
            # schedule['day'] = trackrecord[0][0]
            # schedule['status'] = trackrecord[0][3]
            # schedule['comment'] = trackrecord[0][4]
            if len(trackrecord) < 15:
                return evaluateSchedule(df_ven, df_veri, visiting, lecturers_id, students_id, courseunit, lecturerFree, studentNotFree, venueData, metadata, trackrecord)
    ...
    if len(lecturer_day) > 0:
        ...
        if len(dfs) > 0:
            for row in dfs.itertuples():
                if end_time <= row.starttime or start_time >= row.endtime:
                    verifier[0] = True
                    break
        else:
            verifier[0] = True
...
def scheduleMeetingVenue(df, venueData, meetstart, meetend, meetday, metadata, Weights, trackrecord):
    # get all venues
    ...
    if len(VenueDB) > 0:
        for thisvenue in VenueDB.itertuples():
            if len(df) > 0:
                dfc = df[(df['venue'] == thisvenue.code)&(((df['starttime'] >= meetstart)&(df['starttime'] <=  meetend))|((df['endtime'] >=  meetstart)&(df['endtime'] <=  meetend)))]
                if len(dfc) > 0:
                    continue
                else:
    ...
    if len(freeVenues) > 0:
        for freev in freeVenues:
            ...
            D = int(Fct in freev[4])
            E = int(Dpt in freev[5])
    ...
    if scheduleVenue == '' and metadata < 10:
        if metadata < 5:
            print('VENUE: ',scheduleVenue)
            # Constant = int(sum(Weights)/len(Weights))
            # Weights = [Aw*randrange(1, 10),Bw*randrange(1, 10),Cw*randrange(1, 10),Dw*randrange(5, 12),Ew*randrange(1, 10),Fw*randrange(1, 10)]
            Weights[3] = 10
            random.shuffle(Weights)
            return scheduleMeetingVenue(df, venueData, meetstart, meetend, meetday, metadata+1, Weights, trackrecord)
        else:
...
class Generator():
    ...
    def startgen(gendata, dbinstance):
        ...
        if len(tableData) > 0:
            ...
            for row in df.itertuples():
                ...
                for studentx in student:
                    if not(row.groupid in scheduleRegistry):
                        ...
                        schedule = evaluateSchedule(df[df['venue'] != ''], df[df['status'] == True], row.visiting, lecturers, studentx, row.courseunit, lecturerFree, studentNotFree, venueData, 0, [])
            ...
            # print(df_gen)
            message = '<div class="alert alert-success col-12" role="alert" style="margin-bottom: 0;line-height: 1.2;font-size: 14px;"><i class="mdi mdi-checkbox-marked-circle-outline"></i> '+str(successSchedule)+' out of '+str(totaltable)+' courses have been successfully scheduled. Click view Tables to manage timetable!</div>'
        else:
